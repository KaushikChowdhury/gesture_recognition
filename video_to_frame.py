...
"""
convert given gestures and its videos to frames to be fed into model
"""
...
def convert(gesture_folder, target_folder):
    ...
    for gesture in tqdm(gestures, unit='actions', ascii=True):
        ...
        os.chdir(gesture_path)   # change the working directory to the specific gesture video
        ...
        for video in tqdm(videos, unit='videos', ascii=True):
            ...
            # assuming first 30 frames are important
            while count <= 30:
                ret, frame = cap.read()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = "Extracting individual frames from gesture videos")
